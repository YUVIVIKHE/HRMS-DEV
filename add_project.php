...
/**
 * Add/Edit Project
 * Requirement: Project Budgeting Module (Module 6)
 * Purpose: Admin can create and edit projects with budget allocation
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($is_edit) {
        ...
        $stmt = $conn->prepare("UPDATE projects SET project_name = ?, project_code = ?, start_date = ?, end_date = ?, budget_amount = ?, status = ?, description = ?, updated_at = NOW() WHERE id = ?");
        $stmt->bind_param("ssssdssi", $project_name, $project_code, $start_date, $end_date, $budget_amount, $status, $description, $project_id);
        
        if ($stmt->execute()) {
            $success = 'Project updated successfully!';
            // Refresh project data
            $stmt2 = $conn->prepare("SELECT * FROM projects WHERE id = ?");
        ...
        // Create new project
        $stmt = $conn->prepare("INSERT INTO projects (project_name, project_code, start_date, end_date, budget_amount, status, description, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssdssi", $project_name, $project_code, $start_date, $end_date, $budget_amount, $status, $description, $admin_id);
...
<head>
    ...
    <title><?php echo $is_edit ? 'Edit' : 'Add'; ?> Project - HRMS</title>
        ...
        <aside class="sidebar" id="sidebar">
            ...
            <nav class="sidebar-nav">
                ...
                <?php elseif ($user_type === 'manager'): ?>
                    ...
                    <a href="team_attendance.php" class="nav-item <?php echo $current_page == 'team_attendance.php' ? 'active' : ''; ?>">
                        <svg class="nav-icon" viewBox="0 0 20 20" fill="currentColor">
                            <path fill-rule="evenodd" d="m10 18a8 8 0 1 0 0-16 8 8 0 0 0 0 16zm1-12a1 1 0 1 0-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 1 0 1.415-1.415l-2.121-2.122v-3.999z"/>
                        </svg>
                        <span class="nav-text">Attendance</span>
                    </a>
        ...
        <main class="main-content">
            <header class="top-header">
                <div class="header-left">
                    ...
                    <h1 class="page-title"><?php echo $is_edit ? 'Edit' : 'Add New'; ?> Project</h1>
                </div>
                        ...
                        <div class="form-step active">
                            ...
                            <div class="form-grid">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="project_name" class="form-input" placeholder="Enter project name" value="<?php echo $project ? htmlspecialchars($project['project_name']) : ''; ?>" required>
                                </div>
                                <div class="form-group">
                                    <label class="form-label">Project Code <span class="required">*</span></label>
                                    <input type="text" name="project_code" class="form-input" placeholder="e.g., PROJ-001" value="<?php echo $project ? htmlspecialchars($project['project_code']) : ''; ?>" required>
                                </div>
                                <div class="form-group">
                                    <label class="form-label">Start Date <span class="required">*</span></label>
                                    <input type="date" name="start_date" class="form-input" value="<?php echo $project ? $project['start_date'] : ''; ?>" required>
                                </div>
                                <div class="form-group">
                                    <label class="form-label">End Date</label>
                                    <input type="date" name="end_date" class="form-input" value="<?php echo $project ? $project['end_date'] : ''; ?>">
                                </div>
                                <div class="form-group">
                                    <label class="form-label">Budget Amount (₹) <span class="required">*</span></label>
                                    <input type="number" name="budget_amount" class="form-input" placeholder="0.00" step="0.01" min="0" value="<?php echo $project ? $project['budget_amount'] : ''; ?>" required>
                                </div>
                                <div class="form-group">
                                    <label class="form-label">Status <span class="required">*</span></label>
                                    <select name="status" class="form-input" required>
                                        <?php 
                                        $selected_status = $project ? $project['status'] : 'Planning';
                                        echo renderDropdownOptions($conn, 'project_status', $selected_status, false);
                                        ?>
                                    </select>
                                </div>
                                <div class="form-group full-width">
                                    <label class="form-label">Description</label>
                                    <textarea name="description" class="form-input" rows="4" placeholder="Enter project description"><?php echo $project ? htmlspecialchars($project['description']) : ''; ?></textarea>
                                </div>
                            </div>
                            ...
                            <div class="form-actions" style="margin-top: 24px; display: flex; gap: 12px;">
                                ...
                                <a href="projects.php" class="btn-secondary" style="text-decoration: none; display: inline-flex; align-items: center; padding: 10px 20px;">
                                    Cancel
                                </a>
                            </div>
                        </div>
